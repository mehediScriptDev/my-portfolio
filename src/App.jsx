...
import './App.css'
import image from "../src/Components/images/profile1.png";
import "animate.css";
...
const showhide = ()=>{
  ...
  <img
          className="lg:w-52 lg:h-52 w-40 h-40 hover:border-textcl animate-fade-loop border-textcl border-2  object-cover rounded-full"
          src={image}
          alt="myimage"
        />
      ...
      <div>
        <div className="mt-1">
          <div className="flex justify-center pb-3 items-center gap-2 md:gap-3 mt-3">
            <Link
              ...
              target="_blank"
              className="bg-secondary hover:text-textcl text-xl text-white duration-200 shadow-md px-3 py-3 rounded-full"
            >
              <FaGithub></FaGithub>
            </Link>
            <Link
        ...
        <div className="flex justify-center items-center mt-1">
          <button className="btn btn-md px-5 font-mont font-medium mb-3 sm:mb-0 lg:mb-0 rounded-md  hover:border-anotherbg bg-anotherbg hover:text-anotherbg text-primary shadow-none opacity-85  duration-300 hover:border-2 hover:bg-transparent transition-all border-transparent">
            RESUME <BsDownload />
          </button>
        </div>
      </div>
      <Link  className="flex dimlight justify-center  items-center mt-3 ">
        <button onClick={showhide} className="btn w-full sm:w-full rounded-md px-10  hover:text-white hover:bg-textcl text-textcl duration-300 bg-transparent border-textcl border-[1px] shadow-none  group">
          <p>Enter Portfolio</p>
      ...
      <div className={`${hidd? 'block':'hidden'}`}>
        <Home></Home>
      </div>
    </section>
  )
